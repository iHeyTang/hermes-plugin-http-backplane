...
``hermes-integration`` CLI (which runs in a separate process) can ask
the live backplane to re-register or unregister an integration after
it has changed the files under ``~/.hermes/integrations/<name>/``.
