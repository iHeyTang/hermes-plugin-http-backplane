...
这是一个三仓 stack 的**后端 HTTP 层**：

| Repo | 角色 |
|---|---|
| **this repo** | 本地 HTTP server (`/hermes/*` + `/integrations/<name>/*`) |
| [hermes-plugin-browser-tools](https://github.com/iHeyTang/hermes-plugin-browser-tools) | 给 agent 的 browser 工具（screenshot / navigate / inbox 等），通过 WS bridge 连扩展 |
| [hermes-my-browser-extension](https://github.com/iHeyTang/hermes-my-browser-extension) | Chrome 扩展前端，调本插件的 `/hermes/*` 端点 |

最少使用场景：只装本插件就有 `/hermes/*` API 表面（cron / sessions / model
config / skills / memory / status / lifecycle 全套），可以拿 curl 或自己写
客户端直接用，**不强依赖另外两个仓**。但浏览器扩展依赖本插件，没有就跑不
起来。

```bash
hermes plugins install iHeyTang/hermes-plugin-http-backplane
hermes plugins install iHeyTang/hermes-plugin-browser-tools   # 可选，但配套用
hermes chat
```
